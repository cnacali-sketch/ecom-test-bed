...
@router.patch("/{customer_id}", response_model=CustomerRead, dependencies=[Depends(require_admin)])
async def update_customer(
    customer_id: uuid.UUID, payload: CustomerEdit, db: AsyncSession = Depends(get_db_session)
) -> User:
...
@router.patch("/{customer_id}/block", response_model=CustomerRead, dependencies=[Depends(require_admin)])
async def block_customer(
    customer_id: uuid.UUID, payload: BlockRequest, db: AsyncSession = Depends(get_db_session)
) -> User:
